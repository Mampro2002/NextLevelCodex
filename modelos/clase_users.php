...
class Users
{
    ...
    function añadir($user_new, $name, $email, $pass, $level, $options)
    {
        ...
        if ($options == 3) {
            ...
            $conectado = 1;
            $filt = $db->prepare("UPDATE usuarios SET ultima_conexion = NOW(), conectado = ? WHERE id = ?");
            $filt->bind_param("ii", $conectado, $vec["id"]);
            $filt->execute();
